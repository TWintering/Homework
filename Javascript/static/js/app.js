...
function sortDate(){  
  ...
  if (sortDate.times == undefined){ //holding state in function property to decide whether to sort ascending or descending
    sortDate.times = 0;}
  else {sortDate.times++;}
  filteredData.forEach(function(date) {
    ...
    var numdate =parseInt(newdate,10); //creating numerical date to sort
    date.numdate = numdate;
  });
  if (sortDate.times % 2 == 0){
    filteredData.sort((a,b) => a.numdate - b.numdate);}  
  else {filteredData.sort((a,b) => b.numdate - a.numdate);}  //sort the other way if button clicked again
  filteredData.forEach(obj => delete obj.numdate); //remove numeric date because we don't want it in displayed table
  makeTable(filteredData);
}
...
function sortCity(){
  ...
  if (sortCity.times % 2 == 0){
    filteredData.sort((a,b) => (a.city > b.city) ? 1 : ((b.city > a.city) ? -1 : 0));} // string comparison function
...
function sortDuration(){
  ...
  filteredData.forEach(obj => delete obj.number); //remove temporary property
  makeTable(filteredData);
}
